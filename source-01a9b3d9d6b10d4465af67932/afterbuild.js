...
function run() {
  if (fs.existsSync(pp("build"))) {
    ...
    const matchJs = htmlContent.match(
      /<script[^<>]+src=['"]\/university\/static\/js\/main\S+.js['"][^<>]*><\/script>/g
    );
    const matchCss = htmlContent.match(
      /<link[^<>]+href=['"]\/university(\/static\/css\/main\S+.css)['"][^<>]*>/
    );
